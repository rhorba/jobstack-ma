...
## 2026-07-21 — Sprint 9 Story 9.1 EXECUTE (DevOps/DevSecOps)
- Added `docker-compose.prod.yml`, `frontend/Dockerfile.prod`, `frontend/nginx.prod.conf`, `frontend/docker-entrypoint-tls.sh`: TLS-terminating nginx (self-signed cert generated on first boot into a named volume, HTTP->HTTPS 301 redirect, HSTS header), nginx now runs non-root (`nginxinc/nginx-unprivileged`, uid 101) instead of stock root nginx; api/db were already non-root (uid 100 `app` / uid 70 `postgres`). Prod compose requires real `DB_NAME`/`DB_USER`/`DB_PASSWORD` in `.env` (`${VAR:?...}`, no `changeme` fallback).
- Documented the self-signed -> real-cert swap-in procedure in `docs/devops-jobstack.md` §7; added optional `APP_DOMAIN` var to `.env.example` (cert CN, defaults to `jobstack.local`).
- Live-verified: `docker compose -f docker-compose.prod.yml build && up -d` — confirmed HTTP 301->HTTPS, HSTS header present, cert CN correct, `/api/v1/jobs` reachable through the HTTPS proxy (200), `docker top` confirmed non-root for nginx/db/api, and confirmed the stack refuses to start with `.env` temporarily removed (fails loud on the missing required vars, as intended). Stack torn down cleanly after verification.
- Bug found+fixed during verification: prod/dev compose files collided on the same image tag (`jobstack-ma-nginx`) — added `name: jobstack-prod` to `docker-compose.prod.yml` to isolate images/volumes/networks from the dev stack. Full root-cause in corrections.md.

## 2026-07-21 — Sprint 9 Story 9.2 EXECUTE (Test Architect + Tester)
- Added a new `e2e/` Playwright project (not part of the frontend Angular workspace — separate `package.json`, `@playwright/test` + `dotenv`) with `e2e/playwright.config.ts` (loads `../.env` for `ADMIN_SEED_EMAIL`/`ADMIN_SEED_PASSWORD`, records video on every run) and one spec, `e2e/tests/release-v1.spec.ts`, driving all three Story 9.2 flows in a single continuous browser session (one video, one file): employer registers -> creates company -> posts a job -> mock-CMI-success checkout -> job goes LIVE; candidate registers -> completes profile -> uploads a CV (`e2e/fixtures/sample-cv.pdf`, a minimal hand-built valid PDF) -> searches/finds the new posting -> applies; admin (seeded account) opens the moderation queue -> approves the posting.
- Ran live against the rebuilt dev docker-compose stack (port 8090): **1 passed** on the first real run, ~9s. Saved the recording to `.recordings/v1.0-2026-07-21.webm` per CLAUDE.md rule 9 / Story 9.2's AC. Dev stack torn down cleanly after the run.
- `.gitignore` updated for `e2e/node_modules/`, `e2e/test-results/`, `e2e/playwright-report/`.
